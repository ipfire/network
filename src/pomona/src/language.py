...
#              pomona runtime language choice and installed system
#              language support.
#
...
from pyfire.translate import cat
...
class Language:
	def __init__ (self):
		...
		search = ('../lang-names', 'lang-names', '/usr/lib/pomona/lang-names')
		for path in search:
		...
		search = ( '../lang-table', 'lang-table', '/etc/lang-table', '/usr/lib/pomona/lang-table')
		for path in search:
			if os.access(path, os.R_OK):
				f = open(path, "r")
				break
		...
		# Set the language for pomona to be using based on current $LANG.
